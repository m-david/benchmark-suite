...
-DLOCATOR_HOST=$LOCATOR_HOST \
-DLOCATOR_PORT=$LOCATOR_PORT \
-Dlog4j.configurationFile=$APP_HOME/conf/log4j2.xml \
-Dbenchmark.record.count=$RECORDS \
-Dbenchmark.batch.size=5000 \
-Dbenchmark.range.percent=0.05 \
-Dbenchmark.mode=$MODE \
-Dbenchmark.payload.size=$SIZE \
$MEM_OPTS $GC_OPTS"
...
COMMAND_LINE="java $JAVA_OPTS -cp $CLASS_PATH org.openjdk.jmh.Main GeodePayloadBenchmark $JMH_OPTS"
